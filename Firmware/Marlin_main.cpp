...
void failstats_reset_print()
{
	...
	eeprom_update_word((uint16_t *)EEPROM_FCHECK_COUNT, 0);
}
...
if((eSoundMode==e_SOUND_MODE_LOUD)||(eSoundMode==e_SOUND_MODE_ONCE))
			...
			eeprom_update_word((uint16_t *)EEPROM_FCHECK_COUNT_TOT, 0);
...
/*	if (mcu & 1) SERIAL_ECHOLNRPGM(_T(MSG_POWERUP));
  ...
  if (1) {
	...
	if (eeprom_read_word((uint16_t*)EEPROM_FCHECK_COUNT) == 0xffff) eeprom_write_word((uint16_t*)EEPROM_FCHECK_COUNT, 0);
	...
	if (eeprom_read_word((uint16_t*)EEPROM_FCHECK_COUNT_TOT) == 0xffff) eeprom_write_word((uint16_t*)EEPROM_FCHECK_COUNT_TOT, 0);
#ifdef SNMM
