...
//////////////////////
	...
	uint16_t filam_check = eeprom_read_word((uint16_t*)EEPROM_FCHECK_COUNT_TOT);
    uint16_t crashX = eeprom_read_word((uint16_t*)EEPROM_CRASH_COUNT_X_TOT);
    uint16_t crashY = eeprom_read_word((uint16_t*)EEPROM_CRASH_COUNT_Y_TOT);
	lcd_printf_P(PSTR(ESC_H(0,0) "Total failures" ESC_H(1,1) "Power failures  %-3d" ESC_H(1,2) "Fil. %d / %d" ESC_H(1,3) "Crash  X %-3d  Y %-3d"), power, filam, filam_check, crashX, crashY);
...
//////////////////////
	...
	uint16_t filam_check = eeprom_read_word((uint16_t*)EEPROM_FCHECK_COUNT);
    uint8_t crashX = eeprom_read_byte((uint8_t*)EEPROM_CRASH_COUNT_X);
    uint8_t crashY = eeprom_read_byte((uint8_t*)EEPROM_CRASH_COUNT_Y);
	lcd_printf_P(PSTR(ESC_H(0,0) "Last print failures" ESC_H(1,1) "Power failures  %-3d" ESC_H(1,2) "Fil. %d / %d" ESC_H(1,3) "Crash  X %-3d  Y %-3d"), power, filam, filam_check, crashX, crashY);
	menu_back_if_clicked_fb();
}
/**
