...
int8_t SilentModeMenu = SILENT_MODE_AUTO;
#else
...
/*RAMPS*/  MENU_ITEM_FUNCTION_P(PSTR("Hotend - " STRINGIFY(PREHEAT_HOTEND_TEMP) "/0"), lcd_preheat_nozzle);
/*RAMPS*/  MENU_ITEM_FUNCTION_P(PSTR("Bed    - " "0/" STRINGIFY(PREHEAT_HPB_TEMP)), lcd_preheat_bed);
...
void lcd_bed_calibration_show_result(BedSkewOffsetDetectionResultType result, uint8_t point_too_far_mask)
{
    ...
    if (result == BED_SKEW_OFFSET_DETECTION_POINT_NOT_FOUND) {
        ...
        if (point_too_far_mask == 0){
			...
			//SERIAL_ECHOPGM("point_too_far_mask: ");
			//MYSERIAL.print(point_too_far_mask);
			/*test-end*/
        ...
        if (point_too_far_mask == 0 || result > 0) {
            switch (result) {
                default:
                    // should not happen
					/*test*/ //msg = _i("point_too_far_mask == 0 || result > 0");
					msg = _T(MSG_BED_SKEW_OFFSET_DETECTION_FITTING_FAILED);
                    break;
...
#else //TMC2130
	if (!farm_mode) { //dont show in menu if we are in farm mode //or if using RAMPS /*ramps/*
		if (MOTHERBOARD != BOARD_RAMPS_14_EFB)
		{
			switch (SilentModeMenu) {
		...
		{
			eeprom_update_byte((unsigned char *)EEPROM_SILENT, SILENT_MODE_AUTO); /*ramps*/ //use auto mode as default :)
		}
	}
#endif //TMC2130
	...
	while (!endstop_triggered) {
		if (((READ(X_MIN_PIN) ^ X_MIN_ENDSTOP_INVERTING) == 1) ||
			...
			if (current_position_init - 1 <= current_position[axis] && current_position_init + 1 >= current_position[axis]) {
				...
				if (MOTHERBOARD == BOARD_RAMPS_14_EFB)
				{
					//current_position[axis] += (axis == X_AXIS) ? 12 : 6;
					current_position[axis] += (axis == X_AXIS) ? BED_X0 : BED_Y0;
				}
				else
				{
